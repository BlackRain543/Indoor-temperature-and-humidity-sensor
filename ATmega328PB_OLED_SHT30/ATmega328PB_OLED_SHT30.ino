...
SoftwareSerial TuyaSerial(0, 1); //MCU_RX, MCU_TX,与Tuya模块通信
...
void setup(void){
  ...
  TuyaSerial.begin(9600);               //tuya串口初始化
...
void LEDs(SHT31D result){
    ...
    if(result.rh<60.0 && result.t<25.0){
...
// 串口发送
void Uart_PutChar(unsigned char value){
  TuyaSerial.write(value);
}

// 串口接收
void Uart_Receive(){
  if (TuyaSerial.available()) {
    unsigned char value = TuyaSerial.read();
    uart_receive_input(value);
    MySerial.print(value);        //调试
  }
}
